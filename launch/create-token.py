"""
Flash Loan Bot Token Launch — PumpPortal API

Creates token on pump.fun via PumpPortal's local transaction API.
Your private key never leaves your machine.

Requirements:
- pip install solders requests
- Solana wallet with ~0.1 SOL (creation + initial dev buy)
- Token image at assets/flashbot-token.png

Usage:
    python create-token.py [--dev-buy 0.05] [--dry-run]
"""
...
TOKEN_DESCRIPTION = (
    ...
    "Scans Jupiter + Raydium for cross-DEX arb opportunities. "
...
TOKEN_IMAGE = ASSETS_DIR / "flashbot-token.png"
...
def main():
    parser = argparse.ArgumentParser(description=f"Launch {TOKEN_NAME} ({TOKEN_TICKER}) on pump.fun")
    ...
    print("=" * 50)
    print(f"{TOKEN_NAME} ({TOKEN_TICKER}) TOKEN LAUNCH")
    print("=" * 50)
    print()
    ...
    min_required = args.dev_buy + 0.01
    if balance_sol < min_required:
        print(f"ERROR: Need at least {min_required} SOL")
        sys.exit(1)
    ...
    print(f"  Wallet: {balance_sol:.4f} SOL")
    print(f"{'=' * 50}")
